...
library(sp)
rm(list=ls())
data<-read.csv("C:/Users/fisol/OneDrive/Pictures/RIsk/datarisk2.csv")
data$stunting1
#data
#length(data)
...
n=length(data$wasting1)
d<-list(
  N_Urban=c(data$Urban,rep(NA,(data$Urban)),rep(NA,(data$Urban))),
  N_prim=c(data$prim, rep(NA,data$prim)), rep(NA,data$prim))),
  N_sec=c(data$sec,rep(NA,data$sec)), rep(NA,data$sec))),
  N_high=c(data$high,rep(NA,data$high)), rep(NA,data$high))),
  N_water=c(data$water,rep(NA,data$water)), rep(NA,data$water))),
  N_toilet=c(data$toilet,rep(NA,data$toilet)), rep(NA,data$toilet))),
  N_hhmember56=c(data$hhmember56,rep(NA,data$hhmember56)), rep(NA,data$hhmember56))),
  N_hhmember7=c(data$hhmember7,rep(NA,data$hhmember7)), rep(NA,data$hhmember7))),
  N_Poorer=c(data$Poorer,rep(NA,data$Poorer)), rep(NA,data$Poorer))),
  N_Middle=c(data$Middle, rep(NA,data$Middle)), rep(NA,data$Middle))),
  N_Richer=c(data$Richer,rep(NA,data$Richer)), rep(NA,data$Richer))),
  N_Richest=c(data$Richest,rep(NA,data$Richest)), rep(NA,data$Richest))),
  N_breastfeeding=c(data$breastfeeding,rep(NA,data$breastfeeding)), rep(NA,data$breastfeeding))),
  N_married=c(data$married,rep(NA,data$married)), rep(NA,data$married))),
  N_bord_23=c(data$bord_23,rep(NA,data$bord_23)), rep(NA,data$bord_23))),
  N_bord_4=c(data$bord_4,rep(NA,data$bord_4)), rep(NA,data$bord_4))),
  N_Male=c(data$Male,rep(NA,data$Male)), rep(NA,data$Male))),
  N_fever=c(data$fever,rep(NA,data$fever)), rep(NA,data$fever))),
  N_cough=c(data$cough,rep(NA,data$cough)), rep(NA,data$cough))),
  N_diarrhoea=c(data$diarrhoea,rep(NA,data$diarrhoea)), rep(NA,data$diarrhoea))),
  N_contraceptive=c(data$contraceptive,rep(NA,data$contraceptive)), rep(NA,data$contraceptive))),
#### adding more fixed effect variables####
  N_child_age=c(data$child_age,rep(NA,data$child_age)),rep(NA,data$child_age))),
  N_mother_age=c(data$mother_age,rep(NA,data$mother_age)), rep(NA,data$mother_age))),
### Response stunting and underweight
  OBS=c(data$stunting1,rep(NA,data$underweight1)), rep(NA,data$wasting1))),
  idx=N_idx
)
d.add=1E-4
shyper=list(prec=list(prior="pcprec",param=c(1,0.001)))
...
formula<-OBS ~ 0 +N_Urban+N_prim+N_sec+N_high+N_water+N_toilet+N_hhmember56+N_hhmember7+
 N_Poorer+N_Middle+N_Richer+N_Richest+N_breastfeeding+N_married+N_bord_23+N_bord_23+
...
library(xtable)
xtable(IMCAR$summary.fixed[,c("mean","sd","0.025quant","0.975quant")],digits=3)
...
plot((data$state[3]), data$stunting1)
